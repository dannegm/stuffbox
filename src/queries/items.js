...
export const itemsAtLocationQuery = (locationId, opts = {}) => ({
    ...
    queryFn: async () => {
        const { data, error } = await supabase()
            .from('items')
            .select(
                'id, name, quantity, icon, active_move_id, created_at, purchase_price, item_photos(r2_key, order, crop_x, crop_y, zoom, rotation, flip_x, flip_y), item_tags(tags(id, icon, name))',
            )
