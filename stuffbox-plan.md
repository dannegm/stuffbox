...
- **Avatars:** DiceBear (style TBD — bins uses `rings`, abstract, no gender concept).
...
One schema per project. PKs are `nanoid(8)` text — short, URL/label-friendly, non-sequential (like `bins.bins.id`). Guessing an id grants nothing: everything is behind auth + RLS. **This model is unchanged from the previous plan — Supabase didn't change with the Next switch.**
...
- `app_settings`: **SELECT = any authenticated** (client must read it to apply config); writes admin only. (Admin-only *UI* visibility is frontend-only; the value is readable by design.)
...
**AI box summaries** — label rule: `IF a location has 4+ direct children THEN show an AI summary; ELSE show nothing`. Cached in `locations.ai_summary`, generated on box-open or content change via `POST /api/summary` (Route Handler, holds LLM key), **not** at print time. "Regenerar" action refreshes. ~200 chars.
...
## 13. Dependencies
**New:** `@react-pdf/renderer`, `qrcode`, `react-easy-crop`, `@turf/turf` (or mapcn `Arcs`), `@aws-sdk/client-s3` + `@aws-sdk/s3-request-presigner` (Route Handler side), `resend`, an LLM SDK, DiceBear.
...
- DiceBear style (abstract like bins' `rings` vs human — decides whether `gender` maps to anything). Deferred; doesn't block.
