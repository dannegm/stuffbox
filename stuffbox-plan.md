...
- **Icons:** multi-library — **Phosphor** (`@phosphor-icons/react`, imported from its `/ssr` subpath, code `phosphor`) is the primary set, used everywhere in the actual UI after a full migration off Hugeicons. Hugeicons (`@hugeicons/react` + `@hugeicons/core-free-icons`, code `huge`) was the primary set before that migration and stays available as a `DynamicIcon` option, alongside Lucide (`lucide-react`, code `lucide`) and Lucide Lab (`@lucide/lab`, code `lucide-lab`) for anything ported as-is from pinia/bins. `icon` jsonb `{ library, name }` resolved via one `DynamicIcon` component (`src/ui/dynamic-icon.jsx`, extended from pinia's lucide-only version) that dispatches on `library`. `huge` and `phosphor` names both use the `XxxIcon` suffix convention.
- **Maps:** mapcn (`https://mapcn.dev`) is a real shadcn-style registry, not an npm package — that's why `pnpm info @mapcn/map` 404s. Registered in `components.json` (`registries: { "@mapcn": "https://mapcn.dev/r/{name}.json" }`); installed via `npx shadcn@latest add @mapcn/map`, same as any other shadcn component. The CLI writes it to `src/components/ui/map.jsx` by default (ignores the project's `ui` alias for this particular registry item) — move it to `src/ui/map.jsx` by hand after every reinstall/update to match this project's convention. `src/ui/map.jsx` is the **full** component (`Map`, `useMap`, `MapMarker`, `MarkerContent`, `MarkerPopup`, `MarkerTooltip`, `MarkerLabel`, `MapPopup`, `MapControls`, `MapRoute`, `MapArc`, `MapGeoJSON`, `MapClusterLayer`) with its `lucide-react` icons swapped for Phosphor (matching the icon migration), plus a hand-diverged `ControlGroup`/`ControlButton` treatment (pinia's nicer version: `size-9`, `rounded-lg`, explicit `shadow-md shadow-black/10` per this project's "every `shadow-*` needs an explicit color" rule, `text-foreground/70` for icon contrast in both themes — but hover is `hover:bg-muted`/`hover:text-foreground`, **not** pinia's `hover:bg-accent`/`hover:text-accent-foreground`: this project's `--accent` is a saturated indigo from the theme preset, not a neutral gray, so it read as an out-of-place blue hover on a plain toolbar button) — reapply this diff after any reinstall/update, same as the icon swap and the `ui`-alias move. Basemap is mapcn's own default (Carto positron/dark-matter via `basemaps.cartocdn.com`, theme-aware) — **not** a custom MapTiler style; `NEXT_PUBLIC_MAPTILER_KEY` stays unused in env for now, available if a future map feature wants a custom basemap via the `styles` prop. The position picker's pin (`src/components/locations/marker-pin.jsx`) is adapted from pinia's `PinGlyph` (`src/components/map/pin-glyph.jsx`) — drop-pin glyph + radar pulse (`animate-radar-ping`, `src/css/anims.css`) + lift animation while dragging (`lifted` prop, driven by `MapMarker`'s `onDragStart`/`onDragEnd` in `LocationMapPicker`) — using semantic tokens (`bg-primary`, `bg-muted-foreground/40`) instead of pinia's hardcoded rose/gray so it's dark-mode-correct too.
...
- **Geocoding:** OpenRouteService's `geocode/autocomplete` endpoint (same `NEXT_PUBLIC_ORS_API_KEY`, direct client `fetch`, no Route Handler) powers `AddressSearch` (`src/components/locations/address-search.jsx`) — the search-as-you-type box overlaid on `LocationMapPicker`. Hand-rolled dropdown (not `Popover`), since a combobox that opens on focus/typing rather than a trigger click fights Popover's click-toggle semantics.
...
`id` PK · `workspace_id` (cascade) · `parent_id` → locations self (nullable, cascade) · `name` · `type` (free text — includes `box`/`shelf`/`toolbox`/`baggage` among the presets) · `icon` jsonb `{library,name}` · `lat`,`lng` double (nullable; meaningful on roots for routes) · `active_move_id` → moves (nullable; set when packed) · `ai_summary` text (nullable, cached, regenerable) · `is_container` bool default false (app-derived from `type` at create time — defaults true for box/shelf/toolbox/baggage — user-editable after; gates whether the fields below + photos show in the UI, without needing to be rateable) · `is_item` bool default false (lets the location also appear in the swipe/rate deck, like an item; implies `is_container` true — app-enforced, not a DB constraint — but not the reverse) · `description` (nullable) · `is_fragile` bool default false · `storage_orientation` (ref → option_lists, nullable) · `sentimental_value` smallint 1–5 (nullable, hearts) · timestamps
A location's total price is never stored — `stuffbox.location_total_price(location_id)` (RPC) recursively sums `purchase_price` across every item in its subtree (nested boxes included, any depth), shown for any location regardless of `is_container`/`is_item` — a house or room's total is just as meaningful as a box's.
...
**AI label descriptions** — superseded the original "AI box summaries" spec below before `/api/summary` was ever built. Generated **client-side, at print time**, in `src/services/label-descriptions.js`, using the same BYOK mechanism as tag suggestions (`getAIModel()`/`isAIConfigured()`, the user's own provider key from their profile — not a house-paid server key). No hardcoded child-count gate: the model itself is instructed to return an empty string (shown as no summary) when there's nothing to add beyond the name, for both containers (based on direct children — items + child locations, one level, not recursive) and items (based on their own metadata: notes, quantity, condition, tags, sentimental value). Nothing is cached — `locations.ai_summary` and `POST /api/summary` are unused leftovers from the original spec (kept in schema/docs in case a cached server-side flow is revisited later, but not wired to anything):
...
**Labels (builder → client-side PDF)** — two-step builder: (1) select what's packed in the move; (2) select which of those get a printed label (skip nested boxes). Per label: QR (always) + name + AI-generated summary (see above — item or container, blank if it wouldn't add value) + fragile icon (red, bold) + orientation icon (black, always up). Generated **client-side** (`@react-pdf/renderer` `pdf().toBlob()`), QR client-side (`qrcode`); page size/grid are configurable per-workspace (`labelLayoutSettings`, `src/helpers/label-layout.js`), defaulting to Letter, 2×5 (10/page), ~101.6×50.8mm cells — supersedes this doc's original A4/2×3/~90mm spec. Three actions on the same bytes: **imprimir** (open blob → native print), **descargar** (download), **enviar por correo** (this one needs a Route Handler + Resend, since email sending isn't client-side; it's the only label path touching the server).
...
Design language: shadcn/Base-UI minimalist baseline, with **"modern skeuomorphism"** reserved for the pack/unpack moment (a box that opens/closes as the packing affordance) — everything else flat and quiet (frontend-design skill's "one signature element"). Default entity icons (Phosphor, editable, `src/constants/location-icons.js`): house→`HouseIcon`, room→`DoorOpenIcon`, box→`PackageIcon`, shelf/library→`BooksIcon`, closet→`TShirtIcon`, drawer→`ArchiveIcon`, warehouse→`WarehouseIcon`, kitchen→`ForkKnifeIcon`, bathroom→`BathtubIcon`, office→`BriefcaseIcon`, garage→`GarageIcon`; fallbacks: location→`FolderIcon`, item→`CubeIcon`, tag→`TagIcon`.
...
**Standard:** `next`, `@base-ui/react`, `@tanstack/react-query`, `nuqs`, `tailwindcss` v4, `maplibre-gl` (installed as a dependency of the `@mapcn/map` registry component, not standalone — see §3), `@supabase/supabase-js` + `@supabase/ssr`, `@phosphor-icons/react` (primary icon set — import from its `/ssr` subpath), `@hugeicons/react` + `@hugeicons/core-free-icons` (secondary, `DynamicIcon`-only after the Phosphor migration), `lucide-react` + `@lucide/lab` (secondary, for ported pinia/bins pieces), `vaul` (Drawer, mobile half of `ResponsiveDialog`), `nanoid`, `shadcn`, `class-variance-authority`, `clsx`, `tailwind-merge`, `date-fns`. **No `react-i18next`.**
