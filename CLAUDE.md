...
- **`useConfirm()` over `window.confirm`/`window.alert`** (`src/hooks/use-confirm.js` + `src/providers/confirm-provider.jsx`, backed by `src/ui/alert-dialog.jsx`) — an imperative `confirm({ title, description, confirmLabel, cancelLabel, variant }) => Promise<boolean>` backed by a single `AlertDialog` instance mounted once in the provider stack, same pattern as `TooltipProvider`. `await confirm({...})` before any destructive mutation (delete workspace/location/item/tag, remove member, toggle admin); pass `cancelLabel: null` for an acknowledge-only message (the drag/transfer guardrails in `location/[id]/page.js` — "no puedes mover esto dentro de sí mismo", etc.) instead of a real yes/no. Never use the native `window.confirm`/`window.alert` — they're blocking, unstyled, and don't match the app's dialog language.
...
Separate from `/api/summary` above (which uses an app-wide OpenRouter key from a server env var for the box AI-summary feature): `src/services/ai.js` is a reusable Vercel AI SDK wrapper for **per-user** AI features, where the user supplies their own provider + API key from their profile (`src/components/auth/ai-integration-section.jsx`, under "Inteligencia artificial" on `/profile`). That config (`{ provider, model, keys: { [providerId]: apiKey } }`) lives only in `localStorage` via the `ai` key in `src/services/settings.js` — never sent to Supabase, never touches a server env var. `getAIModel(modelOverride?)` reads it and returns a `LanguageModel` ready for `generateText`/`streamText`/`generateObject` from `ai`; `isAIConfigured()` is a cheap boolean check for feature-gating UI before calling it. Providers: OpenRouter (default — no official first-class slot in the base `ai` package, hence the dedicated `@openrouter/ai-sdk-provider` adapter, same package `/api/summary` uses), OpenAI, Anthropic, Google — each a factory in `PROVIDER_FACTORIES` keyed to match `src/constants/ai-providers.js`'s `AI_PROVIDERS` list (keep both in sync when adding a provider). Anthropic needs the `anthropic-dangerous-direct-browser-access` header since its API otherwise rejects direct browser calls — the other three don't need this. Every call goes straight from the browser to the provider's API using the user's own key; there is no Route Handler in this path. No AI *features* are built on this yet — it's infrastructure for features to come.
